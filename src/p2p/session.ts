...
export class P2PClientProtocol extends TypedEmitter<P2PClientProtocolEvents> {
    ...
    * */
                    ...
                    try {
                        ...
                        if (json !== undefined) {
                            ...
                            if (
                                ...
                                json.payload !== null &&
                                "num" in json.payload &&
                                typeof (json.payload as any).num === "number"
                            ) {
                                const num = (json.payload as any).num;
    ...
    private emitStreamStartEvent(datatype: P2PDataType): void {
        if (datatype === P2PDataType.VIDEO) {
            const channel = this.currentMessageState[datatype].p2pStreamChannel;

            // watchdog to stop streaming when client disconnects
            setTimeout(() => {
                const vs = this.currentMessageState[datatype].videoStream;
                if (vs && vs.listenerCount("data") === 0) {
                    rootP2PLogger.debug("No consumers -> auto-ending stream", {
                        stationSN: this.rawStation.station_sn,
                        channel,
                    });
                    this.endStream(P2PDataType.VIDEO, true);
                }
            }, 3000);
        }
